...
# a home-made stock market value parser.
    ...
    start = source.find("ref_660463_l", start + 1)
...
#infile is the name of the html file that is saved from google's stock page
    ...
    dt = now.strftime("%Y-%m-%d--%H:%M-Amazon")
...
#       begin(sys.argv[1])
    ...
    response = urllib2.urlopen('https://finance.google.com/finance?q=NASDAQ%3AAMZN&ei=H7d0WpnfMdClmAG1yJTQBg')
    data = response.read()
